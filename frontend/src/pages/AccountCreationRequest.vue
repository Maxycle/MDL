	...
	<Container>
		<div class="flex flex-col items-center bg-gradient-to-b from-[#14191d] to-blueLogoLight w-full min-h-screen">
			<div class="w-2/3 md:w-1/3 text-center">
				<h2 class="text-lg sm:text-2xl bg-[#42b883] rounded-t-lg px-4 pt-4 shadow-md text-orange-700 shadow-black/50">Mouvement des
					Libertariens
				</h2>
				...
				<div v-if="errorMessage" class="border-2 border-red-500 rounded-lg bg-red-600 p-4 mb-4 text-white">{{
					errorMessage }}</div>
				<div v-else-if="!sessionStore.getUserId && fullname">
					<div class="border-2 border-red-500 rounded-lg bg-green-600 p-4 mb-4">
						{{ fullname }}, veuillez valider votre demande d'ouverture de compte en cliquant le lien qui vous a été
						envoyé par email.
					</div>
				</div>
				<div v-else-if="sessionStore.getUserId && fullname">
					...
					<form @submit.prevent="sendRequest"
						...
						<div class="flex flex-col gap-1.5 py-2.5 text-left">
							...
							<input v-model="firstname" type="text"
								...
								class="text-xs sm:text-lg bg-transparent border-b border-transparent mx-4 py-2 focus:border-[#42b883aa] focus:outline-none transition-colors duration-300">
						</div>
						...
						<div class="flex flex-col gap-1.5 py-2.5 text-left">
							...
							class="text-xs sm:text-lg"
								:placeholder="sessionStore.getUserId ? 'email du coopté' : 'Votre email...'" name="email" :class="['bg-transparent border-b mx-4 py-2 focus:outline-none transition-colors duration-300',
									emailError ? 'border-red-500 focus:border-red-500' : 'border-transparent focus:border-[#42b883aa]']">
							<div v-if="emailError" class="text-red-400 text-sm mx-4 mt-1">{{ emailError }}</div>
						</div>
						...
						<div class="flex flex-col gap-1.5 py-2.5 text-left">
							...
							<textarea v-model="motivations" name="Motivations"
								...
								class="text-xs sm:text-lg bg-transparent border-b border-transparent mx-4 py-2 min-h-[100px] focus:border-[#42b883aa] focus:outline-none transition-colors duration-300 resize-y"></textarea>
						</div>
